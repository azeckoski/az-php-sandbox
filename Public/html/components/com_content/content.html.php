...
//Susan removed this to allow all categories to appear in the news categories page 
		//	if ( $catid != $row->id ) {
		if ( $row->access <= $gid ) {
	...
	function showTable( &$params, &$items, &$gid, $catid, $id, &$pageNav, &$access, &$sectionid, &$lists ) {
		...
		if ( $params->get( 'headings' ) ) {
			...
			<tr>
				...
				if ( $params->get( 'title' ) ) {
					?>
					<td class="sectiontableheader<?php echo $params->get( 'pageclass_sfx' ); ?>" width="85%">
