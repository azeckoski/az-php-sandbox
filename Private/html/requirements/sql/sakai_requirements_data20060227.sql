...
insert into requirements_data (jirakey,jiranum,summary,description,audience,component) values ('REQ-114','114','Anonymous poll tool','A simple but useful tool is one which can quickly survey opinion on a specific topic - a poll tool.
<br/>

<br/>
The tool should provide a simple multiple-choice question, allowing respondents to choose one or possibly multiple options.
<br/>

<br/>
Aggregate results are immediately available to participants.
<br/>

<br/>
The poll tool should be optionally integrated with discussion (ideally targetted at Message Centre) so that it''s possible to follow a link to a topic / thread about the poll. There may also be other integration points and features, e.g. ability to send an email notification of a new poll, or automatically add an announcement about the poll (with a link to take the poll and/or view results).
<br/>

<br/>
Some screenshots from an existing CMS illustrating the basic idea are attached.','Students|Instructors|Researchers','New Tool');
...
insert into requirements_data (jirakey,jiranum,summary,description,audience,component) values ('REQ-181','181','Ability to specify the order the assignments are listed ','Our instructors would like to be able to specify the order the assignments are listed.  More specifically, our instructors would like the ability to specify the default order of assignments as they appear to students.  Sorting merely allows the student/instructor to change the default order of assignments.  Our instructors need the ability to set the default order and if that order is then changed by sorting, that''s fine.','Instructors','Gradebook');
...
insert into requirements_data (jirakey,jiranum,summary,description,audience,component) values ('REQ-184','184','Enter all assignment grades for one user','The ability to enter in all the grades for a specific user.  This could possiblely be accomplished from the roster screen.  Clicking one of the users would take the site author to a screen providing the ability to enter the grades for all assignments for one particular user.','Instructors','Gradebook');
insert into requirements_data (jirakey,jiranum,summary,description,audience,component) values ('REQ-186','186','Allow comments for each assigned grade','Allow comments for each assigned grade (ie each assignment). 
<br/>

<br/>
- Comments would be optionally visible to students as this allows instructors the ability to make private or &quot;public&quot; (to individual students) comments
<br/>
