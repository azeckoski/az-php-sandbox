	...
	for (var f=0; f<document.forms.length; f++) {
		...
		for (var i=0; i<itemCount; i++) {
			...
			if (thisElement.type.toLowerCase() == "submit") {
				...
				if (validateItem == null) {
					var fieldRegexp = /^([a-zA-Z0-9_]+)_[0-9]+/;
					var fieldRootMatches = fieldRegexp.exec(thisElement.name);
				...
				if (validateItem != null) {
					...
					// clean up the validate string if it doesn't start and end with colons
					if (! validateItem.value.match(new RegExp('^'+gSeparator+'.+'+gSeparator+'$'))) {
...
function validateObject(objInput) {
	...
	if (validateItem == null) {
		var fieldRegexp = /^([a-zA-Z0-9_]+)_[0-9]+/;
		var fieldRootMatches = fieldRegexp.exec(objInput.name);
